...
async function createOrderForCapturedPayment({ payment, razorpayPaymentId, gaClientId }) {
    ...
    const { wasFirstCapture, payment: capturedPayment } = await paymentStore.claimCapture({
    ...
    if (!wasFirstCapture) {
        ...
        for (let attempt = 0; attempt < CAPTURE_RACE_RETRY_COUNT; attempt += 1) {
            ...
            const current = await paymentStore.getPaymentByRazorpayOrderId(payment.razorpayOrderId);
        ...
        // page once it lands.
        const finalCheck = await paymentStore.getPaymentByRazorpayOrderId(payment.razorpayOrderId);
...
async function verifyAndFinalizePayment({ userId, razorpayOrderId, razorpayPaymentId, razorpaySignature, gaClientId }) {
    ...
    const payment = await paymentStore.getPaymentByRazorpayOrderId(razorpayOrderId);
...
async function finalizeFromWebhook({ event, payload }) {
    ...
    const payment = await paymentStore.getPaymentByRazorpayOrderId(paymentEntity.order_id);
...
async function refundOrphanedPayment(payment, refundedBy = "system") {
    ...
    try {
        ...
        await paymentStore.markRefundFailed({ razorpayOrderId: payment.razorpayOrderId, reason: error.message });
...
// automatic sweep.
async function listOrphanedPayments() {
    return await paymentStore.findOrphanedCapturedPayments();
}
...
async function reconcileOrphanedPayments({ olderThanMinutes = 15 } = {}) {
    ...
    const candidates = (await paymentStore.findOrphanedCapturedPayments())
